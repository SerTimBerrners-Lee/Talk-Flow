...
- Элементы настройки горячей клавиши диктовки и живой предпросмотр нажатых клавиш переиспользуются в онбординге и настройках; сохранены применение без перезапуска и проверка конфликтов.
...
| Потоковая диктовка | Да | Для проверенных моделей | Для streaming-моделей |
...
Полная маршрутизация данных, runtime-процессы, ограничения и точки отказа описаны в [архитектуре режимов Облако/API/Локально](docs/architecture/processing-modes.md). Проверяемое пользовательское поведение собрано в [общих бизнес-требованиях](docs/business-requirements.md).
...
- [Архитектура режимов Облако/API/Локально](docs/architecture/processing-modes.md) — технический маршрут диктовки, файлов, созвонов, переводов, LLM и истории.
- [Бизнес-требования Talkis](docs/business-requirements.md) — возможности пользователя, ожидаемое поведение, ограничения и критерии готовности.
- [Принципы аудиопайплайна](docs/audio-pipeline-principles.md) — обязательные инженерные правила для записи, STT, файлов и call-capture.
- [Локальный STT runtime](docs/local-stt-runtime.md) — устройство и контракт управляемого локального распознавания.
